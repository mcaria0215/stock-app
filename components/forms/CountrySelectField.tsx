...
import * as Countries from 'country-flag-icons/react/3x2'; // (O)
...
const CountrySelect = ({ value, onChange, }: { value: string; onChange: (value: string) => void; }) => {
  ...
  const renderFlag = (countryCode: string) => {    
    const Flag = (Countries as any)[countryCode.toUpperCase()];
    if (!Flag) return <span className="w-5" />;
    return <Flag title={countryCode} className="w-5 h-auto shadow-sm" />;
  };
            ...
            <CommandGroup className='bg-gray-800'>
              {countries.map((country) => (
                ...
                >
                  ...
                  />
                  <span className='flex items-center gap-2'>
                    <span>{renderFlag(country.value)}</span>
